...
SELECT *
FROM vendor as v-- not a very attractive table due to ID
INNER JOIN vendor_booth_assignments as vba
	ON v.vendor_id = vba.vendor_id -- brings all the product info
...
SELECT 
count(booth_number) as booth_rent_qty
,vendor_id
...
customer_first_name,
customer_last_name,
ROUND(SUM(quantity*cost_to_customer_per_qty),2) as amount_spent
...
FROM vendor
;
SELECT * FROM temp_new_vendor  -- the temp file is accessible
;
